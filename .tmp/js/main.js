...
$(document).ready(function(){
	...
	// set on click listener for first section
	$('div#address-input button').click(function(){
		address_done();
	});

	$("div#address-input>input").keyup(function(event){
		if(event.keyCode == 13){
			address_done();
		}
	});
	...
	$("#section3-container .level").click(function(){
	...
	$('#roles-btn').click(function(){
		...
		// scroll to next page
		// whether we want to send the request and display it.
...
function queryAndProcess(){
	...
	var parameters = ["address", "includeOffices" ];

	for(par in parameters){
		if (url[url.length-1] != "?") url = url + "&";

		var key = parameters[par];
		url = url + key + '=' + localStorage.getItem(key);
	}
